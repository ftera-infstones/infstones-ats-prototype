...
function SortableStageItem({ stage, onDelete, onUpdate }: SortableStageItemProps) {
  ...
  const handleSave = () => {
    if (editName.trim()) {
      onUpdate(stage.id, editName.trim());
    }
    setEditing(false);
  };
  ...
  return (
    <div
      ...
      className={`bg-white rounded-xl border border-zinc-200 px-4 py-3 flex items-center gap-3 ${
        isDragging ? 'shadow-lg' : 'hover:border-zinc-300'
      }`}
    >
      {/* Drag Handle */}
      <button
      ...
      {editing ? (
        <div className="flex items-center gap-2 flex-1">
          ...
          />
          <button onClick={handleSave} className="text-indigo-500 hover:text-indigo-700">
            <Check size={16} />
          </button>
          <button onClick={() => { setEditName(stage.name); setEditing(false); }} className="text-zinc-400 hover:text-zinc-600">
            <X size={16} />
          </button>
        </div>
      ) : (
      ...
      {stage.is_default ? (
        <span className="text-xs text-zinc-400 bg-zinc-100 px-2 py-0.5 rounded-full" title="Default stages cannot be deleted">
          Default
        </span>
      ) : (
        <button
...
export default function PipelineSettingsPage() {
  ...
  const handleUpdate = (stageId: string, name: string) => {
    dispatch({ type: 'UPDATE_STAGE', stageId, name });
  };
    ...
    <div className="min-h-screen bg-zinc-50">
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-zinc-900">Pipeline Stages</h1>
            <p className="text-zinc-500 text-sm mt-1">Drag to reorder · Click name to edit</p>
          </div>
          <button
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-semibold text-zinc-900">New Stage</h2>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium text-zinc-700 block mb-1.5">Stage Name *</label>
                <input
                  type="text"
                  value={addForm.name}
              ...
              <div>
                  ...
                  {presetColors.map(color => (
                    <button
                      ...
                      className={`w-7 h-7 rounded-full border-2 transition-all ${
                        addForm.color === color ? 'border-zinc-800 scale-110' : 'border-transparent'
                      }`}
                ...
                <input
                  ...
                  className="h-8 w-16 rounded cursor-pointer border border-zinc-200"
                />
              </div>
            </div>
